...
If that didn't happen - a failing verification workflow, for example - the same is done by hand. Tag new releases in
the ``main`` branch using a semver compatible value, starting with ``v``:
